...
### Why Is Code Reading So Hard?

Good software design — **OOP**, **Design Patterns**, and **Dependency Injection** — makes code more modular, reusable, and maintainable. These are essential practices. However, they also mean that behavior is distributed across many files, interfaces, and abstraction layers. For developers who are **new to a project** (not necessarily junior — even senior engineers joining a new codebase), this creates a real challenge: the logic is split into many puzzle pieces, and you have to put them all together in your head to understand a single flow.
...
│   └── stripeClient.create()              → open file 6
└── notificationService.send()             → open file 7
    └── templateEngine.render()            → open file 8
```

**7 files. 4 interfaces. 3 layers of abstraction. One simple order.**
